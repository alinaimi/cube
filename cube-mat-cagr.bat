...
@echo *** interpolating to "%3" using years: "20%yr1%" and "20%yr2%"

@rem @set yrCAGR=%arg3:~2,2%
@rem for /f "delims=" %%A in ('echo %n1%^| sed "s/%yr1%/%yrCAGR%/g"') do set "n1_new=%%A"
@for /f "delims=" %%A in ('echo %n1%^| sed "s/%yr1%/%arg3:~2,2%/g"') do set "n1_new=%%A"
...
;n1_new = REPLACESTRIC('%n1%','%yr1%','%arg3%',0)
...
RUN PGM=MATRIX PRNFILE="%PRNFILE%"
  ...
  MATI[2] = "%f2%"
  n1_new = REPLACESTRIC('%n1%','%yr1%','%arg3%',0)
  MATO[1] = "%dp1%%n1_new%-(interpolated-to-%arg3%-using-%yr1%-and-%yr2%).mat",
    NAME = %n_name%,
    @ucmt01@ MO = 101 - 101
    @ucmt02@ MO = 101 - 102
    @ucmt03@ MO = 101 - 103
